...
// This is the upstream cmd/server/main.go with one addition: the server is
// built with a TokenProvider (see tokenprovider.go), which is the only way to
// attach credentials to outbound cross-cluster RPCs. temporal.WithTokenProvider
// is a Go option, not a config key, which is why this repo ships its own server
// binary instead of running temporalio/server unmodified.
	...
	app.Commands = []*cli.Command{
		...
		},
		{
			...
			Action: func(c *cli.Context) error {
				...
				// Replication traffic authenticates with a JWT rather than a client
				// certificate. When no OIDC token endpoint is configured the provider
				// is nil and the option is left off entirely - passing a nil-valued
				// concrete type here would still look non-nil to the server and turn
				// every cross-cluster RPC into an error.
				tokenProvider, err := newOIDCTokenProviderFromEnv(logger)
				if err != nil {
					return cli.Exit(fmt.Sprintf("Unable to instantiate replication token provider: %v.", err), 1)
				}
				if tokenProvider != nil {
					logger.Info("Replication token provider enabled.", tag.NewStringTag("token-url", tokenProvider.tokenURL))
					serverOpts = append(serverOpts, temporal.WithTokenProvider(tokenProvider))
				}

				s, err := temporal.NewServer(serverOpts...)
				if err != nil {
